		...
		function __construct() {
			...
			if(isset($_REQUEST["action"])) {
				switch($_REQUEST["action"]) {
                    ...
                    case 'addPropuesta':
                        ...
                        if(isset($_REQUEST['propuesta1']) && $_REQUEST['propuesta1'] == $propuesta){
                            ...
                            if($limite <= $hoy){
                                ...
                                $fechafincamp = $enfrentamientoMapper->getFechaFinCampeonato($enfrentamiento);

                                if($propuesta > $fechafincamp){
                                    SessionMessage::setMessage("No se puede jugar en fechas posteriores al cierre del campeonato.");
                                    $goto = 'Location: index.php?controller=enfrentamientos&idenfrentamiento=' . $_REQUEST['idenfrentamiento'];
                                    header($goto);
                                }
                                else{
                    ...
                    case 'rechazar':
                        if(isset($_REQUEST['fecha']) && isset($_REQUEST['hora'])){
                            $propuesta = date($_REQUEST['fecha'] . " " . $_REQUEST['hora']);
                        
                            if(isset($_REQUEST['propuesta1']) && $_REQUEST['propuesta1'] == $propuesta){
                                SessionMessage::setMessage("El valor nuevo de propuesta debe ser distinto del que quieres rechazar.");
                                $goto = 'Location: index.php?controller=enfrentamientos&action=rechazar&idenfrentamiento=' . $_REQUEST['idenfrentamiento'];
                                header($goto);
                            ...
                            require_once('Views/campeonato/enfRechazarPropuestaView.php');
                            (new EnfRechazarPropuestaView(SessionMessage::getMessage(), SessionMessage::getErrores(),'','',$listaHoras,'',$datos))->render();
                        }
                        break;
